...
use nativelink_config::schedulers::{
    ExperimentalSimpleSchedulerBackend, SchedulerConfig, SimpleSpec,
};
...
fn inner_scheduler_factory(
    ...
    let scheduler: SchedulerFactoryResults = match &scheduler_config {
        SchedulerConfig::Simple { name: _, spec } => {
            simple_scheduler_factory(spec, store_manager, SystemTime::now)?
        }
        SchedulerConfig::Grpc { name: _, spec } => {
            (Some(Arc::new(GrpcScheduler::new(spec)?)), None)
        }
        SchedulerConfig::CacheLookup { name: _, spec } => {
        ...
        SchedulerConfig::PropertyModifier { name: _, spec } => {
            let (action_scheduler, worker_scheduler) =
                inner_scheduler_factory(&spec.scheduler, store_manager)
